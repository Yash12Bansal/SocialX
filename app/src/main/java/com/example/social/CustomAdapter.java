...
public class CustomAdapter extends RecyclerView.Adapter<CustomViewHolder> {
    ...
    public void onBindViewHolder(@NonNull CustomViewHolder holder, int position) {
        holder.text_at_source.setText(headlines.get(position).getPublishedAt()+"  "+headlines.get(position).getSource().getName());
        holder.text_title.setText(headlines.get(position).getTitle());
        holder.text_description.setText(headlines.get(position).getDescription());
        if(headlines.get(position).getUrlToImage()!=null){
            Picasso.get().load(headlines.get(position).getUrlToImage()).into(holder.img);
        }
